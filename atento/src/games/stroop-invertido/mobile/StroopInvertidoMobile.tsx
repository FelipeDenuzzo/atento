...
export const StroopInvertidoMobile: React.FC<StroopInvertidoMobileProps> = ({
  ...
  const [step, setStep] = useState<"instructions" | "playing" | "finished">("instructions");
  ...
  useEffect(() => {
    if (step === "playing") {
      ...
      for (let i = 0; i < TOTAL_TRIALS; i++) {
        if (i > 0 && i % Math.ceil(TOTAL_TRIALS / PROGRESSION.length) === 0 && progIdx < PROGRESSION.length - 1) progIdx++;
        const { activeColors } = PROGRESSION[progIdx];
        const colorSet = COLORS.slice(0, activeColors);
        const congruent = Math.random() < 0.5;
        let word: ColorName, ink: ColorName;
        if (congruent) {
          word = ink = colorSet[Math.floor(Math.random() * colorSet.length)].name;
        } else {
    ...
    if (current + 1 < trials.length) {
      ...
      if (onEnd) {
        ...
        onEnd({
          ...
          wrong,
          accuracy,
          reactionTimes: answers.map((a) => a.rt),
        });
      }
    }
  }, [current, trials.length, answers, onEnd]);
      ...
      {step === "finished" && (
        <div style={{ textAlign: "center" }}>
          ...
          <ul style={{ listStyle: "none", padding: 0, fontSize: 15 }}>
            ...
            <li>Precisão: {answers.length ? ((answers.filter(a => a.correct).length / answers.length) * 100).toFixed(1) : 0}%</li>
            <li>Tempo médio de reação: {answers.length ? Math.round(answers.reduce((acc, a) => acc + a.rt, 0) / answers.length) : 0} ms</li>
          </ul>
        </div>
      )}
    </div>
